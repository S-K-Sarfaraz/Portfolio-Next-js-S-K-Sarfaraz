...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      className={`flex flex-wrap justify-center items-center gap-8 md:gap-10 mx-auto py-6 ${className || ''}`}
    >
      {items.map((item, index) => (
        <a
          ...
          className={`relative bg-transparent outline-none w-[4em] h-[4em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ""
          }`}
        >
          <span
